...
### Changed
- **Asset `sdp-expectation-notifications` (docs only)**: corrections and additions from a live verification session (2026-08-22, Free Edition serverless SDP, CLI v0.297.2, non-development-mode target). Nothing about behavior, defaults, or code changed.
  - **The destination-config claim in 1.12.1 was wrong and is corrected here.** 1.12.1 stated that a Slack notification destination's config is a webhook URL plus an OAuth token and a channel id requiring Slack app-install rights, and that a Teams destination additionally needs an App ID, Auth Secret, Channel URL, and Tenant ID plus Microsoft Copilot Studio and Entra ID permissions. For an alert, a Slack destination takes the incoming webhook URL and nothing else, and so does Teams. Those other fields do exist on the Add-destination form, are marked `(optional)`, and the form's own section heading scopes them to dashboard subscriptions, a different Databricks feature. Verified twice over: the literal field lists of all five destination types on the form, and the stored API config of a Slack destination that then delivered five messages (`url_set: true, oauth_token_set: false, channel_id_set: false`). The 1.12.1 wording came from a documentation-retrieval summary treated as source text; the form and the destination's own stored config are the authority. What stands unchanged from 1.12.1: destinations serve Databricks SQL and jobs only and never the event hook; a destination is a wrapper around a webhook, not a substitute for one; EMAIL is the only type taking plain addresses with no webhook; minting the Slack incoming webhook is still a Slack app install (create an app, enable incoming webhooks, authorize it into the workspace); and a destination alongside a `user_email` on the same address still sends two independent, non-deduplicated notifications. The consequence the correction carries: a Slack destination is genuinely low-friction, one URL, so the benefit is not merely "centralizing a credential instead of pasting it into every alert." Fixed in the asset README, the backstop alert resource comment, and the companion skill's `adapt-the-pattern.md`.
  - **"A hook cannot send email anyway (no email surface exists inside a hook)" re-scoped to the accurate claim.** There is no Databricks-native email surface inside a hook, but the hook reaches arbitrary HTTPS: the shipped, unmodified hook POSTed successfully to two unrelated third-party collectors (different operators), HTTP 200, the request arriving bare at one of them with no proxy header, no policy header, and no interception signature. Three limits travel with that finding wherever it appears: it was measured with no egress controls configured, so in a workspace with an egress firewall, a network policy, or private-link networking the network policy decides rather than the hook; an authenticated email API is reachable only after editing the hook, because the shipped `requests.post` passes no `headers` argument; and the governed-workspace case could not be tested from Free Edition and is now listed as the most important open question attached to the result.
  - **The `generic` channel format is live-tested.** Previously shipped as documented-but-never-live-tested. All six fields (`text`, `pipeline`, `dataset`, `expectation`, `failed_records`, `passed_records`) arrived verbatim at a third-party collector, the two counts as JSON numbers. The `teams` format remains documented and not live-tested, unchanged.
  - **`Empty result state: Error` explained where the sweep query invites edits.** An alert deployed from this asset's resource reads `Empty result state: Error` in its Advanced panel (nothing in the resource sets it), so a query returning no rows flips the alert to ERROR, which emails on every evaluation. The shipped query's `COALESCE(SUM(...), 0)` guarantees exactly one row and cannot hit this; that property was previously unexplained, so a `GROUP BY`, a `HAVING`, or a filter matching nothing would have quietly converted the backstop into a per-evaluation ERROR emailer. Documented in the alert resource, the in-bundle doc's threshold guidance, and the skill's adaptation reference.
  - **Asserted numbers replaced with measured ones.** The webhook POST costs 116 ms on success and 3 ms to fail on an unresolvable name (instrumented), which is what makes one call per event affordable against the single-digit-second teardown grace budget. The delivery-failure line now ships with a real instance instead of a described shape (`WEBHOOK_DELIVERY_FAILED: ConnectionError: HTTPSConnectionPool(...) ... NameResolutionError ... [Errno -2] Name or service not known`), named as `requests.exceptions.ConnectionError` wherever it appears, since `type(e).__name__` renders it bare and collides with the Python builtin. One notification per state transition is now shown by the product rather than inferred from counting emails: the alert History panel's `Notified destinations` column is empty on evaluations that did not change state.
  - **New: the same violation produces three different message shapes.** The hook webhook carries dataset, expectation name, and exact failed/passed counts; the backstop email carries the evaluated metric against its threshold plus an embedded five-run mini history; a backstop Slack destination carries a state-change title and two deep links and no data at all. Choosing a delivery lane, and a destination type inside it, is therefore also choosing how much a responder can triage before clicking through, which the asset did not surface anywhere. Also documented: the email's `Last 5 runs` block lists evaluations, not notifications, so rows that notified nobody appear in it, and the email subject is `Databricks SQL Alert: <alert display name> (<STATE>)`, so the state is filterable from the subject line alone.
  - **Managing notification destinations requires workspace admin**, stated as documented rather than as stress-tested: the verification account is the sole owner and admin of that workspace, so no blocked case was ever exercised.
  - **The skill gains the CLI delete behavior** alongside the create quirk it already documents (which still reproduces exactly on CLI v0.297.2): a delete takes effect immediately but `list` is eventually consistent, so a "did it work" check should use `get` rather than `list`, and a redundant delete errors with `Error: Webhook id does not exist` rather than being idempotent. This replaces the propagation-lag guess two earlier sessions had made in opposite directions.
  - **Stale comment fixed in `event_log_queries.sql`.** Its `hook_progress` comment still claimed the events record only enable/disable state and no failures, which 1.12.0 had already corrected everywhere else (`hook_progress` records ENABLED, FAILED, and DISABLED transitions), so the asset shipped a comment contradicting its own README.
