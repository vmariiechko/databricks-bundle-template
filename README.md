...
├── resources/
│   ├── sample_ingestion.job.yml         # Example: Ingestion job
│   ├── sample_pipeline.pipeline.yml     # Example: DLT pipeline
│   └── sample_pipeline_trigger.job.yml  # Example: Pipeline trigger
└── src/
    ├── jobs/
    │   └── ingest_to_raw.py              # Sample job code (replace with yours)
└── pipelines/
    ├── bronze.py           # Sample DLT bronze layer
    └── silver.py           # Sample DLT silver layer
```
