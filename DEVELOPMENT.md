...
### 15. Asset Library as Plugin Layer

**Decision**: Introduce an `assets/` directory at the repo root containing
standalone sub-templates, each installable via
`databricks bundle init --template-dir assets/<name>`. Each asset is
entirely self-contained (own schema, own README, own tests) and does not
share `library/helpers.tmpl` or any other state with the core template.

**Rationale**:

- The core template is one-shot; users need a way to add reusable
  Databricks artifacts (recovery scripts, pipeline skeletons, etc.) to
  existing bundles without re-running full initialization.
- The Databricks CLI's `--template-dir` flag supports this pattern
  natively for Git-URL template sources. This is the same pattern used
  by [databricks/bundle-examples](https://github.com/databricks/bundle-examples/tree/main/contrib/templates/data-engineering).
- Self-contained assets work for **any** Databricks bundle, not only
  ones generated by this template. A user with a hand-written bundle can
  install any asset. Sharing helpers with the core template would couple
  assets to the core and break this portability.
- `assets/` is a peer of `template/`, not a subdirectory of it. The CLI
  only walks the directory it is pointed at, so the two paths never
  interact. No `skip()` call is needed in `template/update_layout.tmpl`
  to hide `assets/` from core-template output.
- CLI collision protection (`failed to initialize template, one or more
  files already exist`) makes additive-only composition safe by default;
  no asset can overwrite existing bundle files.

**Consequence**: Code duplication across assets is accepted (e.g., an
asset that needs `node_type_id` redefines it locally rather than
importing from `library/`). This is the price of asset portability and
has been explicitly agreed on as a trade-off. If the asset library grows
to the point where drift is a real cost, a shared-helpers mechanism
(asset-scoped, not repo-scoped) can be added — but not before.

**See also**:
- [ARCHITECTURE.md §8 — Asset Library / Plugins Layer](ARCHITECTURE.md#8-asset-library--plugins-layer)
- [CONTRIBUTING.md — Adding an Asset](CONTRIBUTING.md#adding-an-asset)
- [ASSETS.md](ASSETS.md) — user-facing catalog
