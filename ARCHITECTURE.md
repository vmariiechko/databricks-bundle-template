...
8. [Asset Library / Plugins Layer](#8-asset-library--plugins-layer)
...
## 8. Asset Library / Plugins Layer

### Problem

Users who have already generated a project with this template, or users
who have their own hand-written Databricks bundle, want reusable
Databricks artifacts (recovery scripts, pipeline skeletons, ingestion
jobs, monitoring dashboards) without copy-pasting code across projects.
The core template is one-shot: it runs once to scaffold a bundle. It
cannot extend an existing bundle after the fact.

### Solution

An **asset library** of self-contained sub-templates living under
`assets/<name>/` at the repo root. Each asset is its own standalone
Databricks custom template with its own
`databricks_template_schema.json` and `template/` tree. Users install
one asset at a time via:
...
Every asset follows seven framework rules, enumerated in full in
[CONTRIBUTING.md → Adding an Asset](CONTRIBUTING.md#adding-an-asset).

In architectural terms:
- **Self-contained.** No shared `library/helpers.tmpl`, no references
  outside the asset directory. Code duplication is accepted as the price
  of portability.
- **Additive only.** Assets create new files under a predictable
  subdirectory. The CLI's built-in collision check errors cleanly if a
  file already exists — no asset can overwrite existing state.
- **Parent-agnostic.** An asset installs into any Databricks bundle,
  whether generated by this template or not.
- **Canonical CLI install path.** Git clone remains an escape hatch for
  readers, but the documented install method is
  `databricks bundle init ... --template-dir ...`.

### Composition with parent bundle

The asset never modifies the parent bundle's `databricks.yml` or
`variables.yml`. Composition happens through the existing glob include
patterns already in conventional bundles (`include: ["resources/*.yml"]`).
If an asset needs a non-conventional include, its README documents the
exact one-liner the user must add manually — the CLI itself cannot
modify existing files.

### Relationship to the core template

The asset library and the core template are peers in this monorepo, not
layered on top of each other. The core template lives in `template/` at
the repo root; the asset library lives in `assets/`. The CLI only walks
the directory it is pointed at — `databricks bundle init .` generates
the core template and never sees `assets/`; `databricks bundle init ...
--template-dir assets/<name>` installs one asset and never sees the
core template. The two paths share no runtime state.

See [DEVELOPMENT.md — Design Decision #15](DEVELOPMENT.md) for the
decision log behind this design and
[ASSETS.md](ASSETS.md) for the end-user catalog.
...
| **SDP** | Lakeflow Spark Declarative Pipelines (formerly DLT) |
| **UC** | Unity Catalog |
