...
3. Delivery: only the recipient can confirm the email arrived (sender `noreply@databricks.com`, subject `Databricks SQL Alert: <alert display name> (<STATE>)`, with the state as a literal suffix). Expected volume, observed live: one email per state transition, plus one recovery email if `notify_on_ok` is true, plus windowed re-sends only if `retrigger_seconds` is set. The alert's History panel in the UI carries a `Notified destinations` column that is empty on evaluations which did not change state, so which evaluations notified is observable without counting inboxes; note the email body's own `Last 5 runs` block lists evaluations, not notifications. Ask the user; record their answer as the delivery confirmation.
